...
export async function getCommercialPlanCode(clerkUserId: string): Promise<CommercialPlan> {
  ...
  const rows = await sql`
    SELECT
      e."planCode" AS "entitlementPlan",
      s."plan"::text AS "subscriptionPlan",
    ...
    LEFT JOIN "subscriptions" s ON s."clerkUserId" = u."clerkUserId"
    LIMIT 1
  ...
  if (entitlement && ["basic", "pro", "accounting_pro", "accounting_max"].includes(entitlement)) {
    if (periodEnd > now) return entitlement;
  }
...
export async function setCommercialPlanEntitlement(args: {
  clerkUserId: string;
  plan: PaidPlan;
  billing?: BillingPeriod | null;
  source: "paddle" | "admin" | "bank_transfer" | "system";
}) {
  const sql = sqlClient();
  await sql`
    INSERT INTO "subscriptionPlanEntitlements" ("clerkUserId", "planCode", "billing", "source", "createdAt", "updatedAt")
    VALUES (${args.clerkUserId}, ${args.plan}, ${args.billing ?? null}, ${args.source}, now(), now())
...
export async function clearCommercialPlanEntitlement(clerkUserId: string) {
  const sql = sqlClient();
  await sql`DELETE FROM "subscriptionPlanEntitlements" WHERE "clerkUserId" = ${clerkUserId}`;
}

export function baseSubscriptionPlan(plan: PaidPlan): "basic" | "pro" {
