...
export function BlogCover({
  ...
  const alt = locale === "en" ? post.coverAltEn : post.coverAlt;
  const readingMinutes = locale === "en" ? post.readingMinutesEn : post.readingMinutes;
  const dateLocale = locale === "en" ? "en-GB" : "sl-SI";
    ...
    <section className="relative overflow-hidden h-[420px] sm:h-[500px]">
      ...
      <div className="relative h-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-end pb-12 sm:pb-16">
        ...
        <div className="flex items-center gap-3 text-sm text-slate-200">
          ...
          <span>·</span>
          <span>{readingMinutes} {locale === "en" ? "min read" : "min branja"}</span>
        </div>
      </div>
    </section>
  );
