...
export async function POST(req:Request){if(!verifyBulkInternalSecret(req.headers.get("x-bulk-secret")))return NextResponse.json({error:"Unauthorized"},{status:401});
 ...
 try{
  ...
  await sql`UPDATE "bulkInvoiceJobs" SET "stage"='extract',"totalInvoices"=${data.children.length},"lockedAt"=NULL,"lastError"=NULL,"updatedAt"=now() WHERE "id"=${data.jobId}`;
  return NextResponse.json({success:true,groups:data.children.length});
