...
export async function GET(req: NextRequest) {
  ...
  const condition = requestedStatus
    ? and(eq(invoiceDocuments.clerkUserId, userId), eq(invoiceDocuments.status, requestedStatus as typeof invoiceDocuments.status.enumValues[number]))
  ...
  const allRows = requestedStatus
    ? await db.select({ status: invoiceDocuments.status, cost: invoiceDocuments.processingCostMicros, processingStartedAt: invoiceDocuments.processingStartedAt, processedAt: invoiceDocuments.processedAt })
      .from(invoiceDocuments).where(eq(invoiceDocuments.clerkUserId, userId)).limit(1000)
    : rows.map((r) => ({ ...r, cost: r.processingCostMicros, processingStartedAt: null }));
  ...
  return NextResponse.json({
    ...
    stats: {
      ...
      estimatedMonthlyCostMicros: costs.reduce((a, b) => a + b, 0),
    },
  });
}
