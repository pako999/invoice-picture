...
function StatusBadge({ status }: { status: Invoice["status"] }) {
  const map = {
    sent:    { bg: "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400", label: "Sent" },
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-slate-700">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">To: {inv.recipientEmail}</p>
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4" role="presentation">
      ...
      >
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600 dark:text-slate-300">
              {count} {count === 1 ? "invoice is" : "invoices are"} selected.
            </p>
          </div>
        </div>
        <div className="rounded-xl border border-red-300 bg-red-50 p-4 text-sm font-semibold text-red-800 dark:border-red-800 dark:bg-red-950/40 dark:text-red-300">
          All selected invoices and their saved archive previews will be permanently removed. This action cannot be undone.
        </div>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          >
            {deleting ? "Deleting..." : `Permanently delete (${count})`}
          </button>
        </div>
      </div>
    </div>
...
export default function InvoicesPage() {
  ...
  async function handleBulkDelete() {
    ...
    try {
      ...
      setBulkDeleteError("The invoices could not be deleted. Please try again.");
    } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white">Sent invoices</h1>
          <p className="text-gray-500 dark:text-slate-400 text-sm mt-1">Browse all sent invoices</p>
        </div>
        <button
          onClick={load}
      ...
      {showRangePicker && (
        <div className="mb-6 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl p-4">
          <p className="text-sm font-bold text-gray-900 dark:text-white mb-3">Pick a date range</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <label className="flex flex-col gap-1">
              <span className="text-xs font-semibold text-gray-500 dark:text-slate-400">From</span>
              <input
                type="date"
                value={customFrom}
            ...
            <label className="flex flex-col gap-1">
              <span className="text-xs font-semibold text-gray-500 dark:text-slate-400">To</span>
              <input
                type="date"
                value={customTo}
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: "Total", value: filtered.length, cls: "bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700" },
          { label: "Sent", value: sent, cls: "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-700 dark:text-green-400" },
          { label: "Errors", value: failed, cls: "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-700 dark:text-red-400" },
        ].map((s) => (
      ...
      {!loading && filtered.length > 0 ? (
        <div className={`mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border p-3 ${selectedIds.size > 0 ? "border-red-300 bg-red-50 dark:border-red-900 dark:bg-red-950/20" : "border-gray-200 bg-white dark:border-slate-700 dark:bg-slate-900"}`}>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm text-gray-500 dark:text-slate-400">{selectedIds.size} selected</span>
            {selectedIds.size > 0 ? (
              <span className="flex items-center gap-1 text-sm font-bold text-red-700 dark:text-red-400">
                <span aria-hidden="true">🚩</span> Selected invoices will be permanently removed.
              </span>
            ) : null}
          </div>
          <button
      ...
      {loading ? (
        ...
        <div className="text-center py-24 text-gray-400 dark:text-slate-500">
          ...
          <p className="text-lg font-semibold">
            {list.length === 0 ? "No invoices sent yet" : "No invoices in selected range"}
          </p>
          <p className="text-sm mt-2">
            {list.length === 0
              ? "Go to the Scan tab and send your first invoice."
              : "Adjust the filter to see more invoices."}
          </p>
        </div>
      ) : (
            ...
            >
              ...
              >
                <input
                  ...
                  aria-label={`Select invoice ${inv.subject}`}
                  className="h-5 w-5 rounded border-gray-300 accent-red-600"
                />
              </label>
              {inv.imageMime === "application/pdf" ? (
                <div className="w-16 h-16 group-hover:w-40 group-hover:h-52 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 flex flex-col items-center justify-center flex-shrink-0 gap-0.5 overflow-hidden transition-all duration-200">
                  {displayInv.imageData || displayInv.previewUrl ? (
                    <iframe
                      ...
                      title={`Preview ${displayInv.filename}`}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 dark:text-slate-400 truncate">
                  <span className="font-medium">To:</span> {inv.recipientEmail}
                </p>
                <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">{fmt(inv.createdAt)}</p>
                {inv.status === "failed" && inv.errorMessage && (
